...
static bool limits_pull_off (axes_signals_t axis, float distance)
{
    ...
    st_prep_stepper_buffer(!(BOARD_OFFLOAD_TO_HOST || BOARD_OFFLOAD_TO_CORE), false);   // Prep and fill segment buffer from newly planned block.
    st_wake_up();       // Initiate motion.

    while(true) {

        st_prep_stepper_buffer(!(BOARD_OFFLOAD_TO_HOST || BOARD_OFFLOAD_TO_CORE), false); // Check and prep segment buffer.
...
static bool limits_homing_cycle (axes_signals_t cycle, axes_signals_t auto_square)
{
    ...
    do {
        ...
        do {
            ...
            st_prep_stepper_buffer(!(BOARD_OFFLOAD_TO_HOST || BOARD_OFFLOAD_TO_CORE), false); // Check and prep segment buffer. NOTE: Should take no longer than 200us.
