...
`grbl_hal_t` gets some new methods to move data among different cores and the host.
...
All calls to `sys` and `hal` in stepper.c are moved and/or adapted for deferred execution. In particular whatever realtime parameters were set in planner block buffer and segment buffer prep, are set in `static stepper_t *st` instead, at the very last moment before going live.
...
### 4.2. Changes to protocol.c, grbllib.c, gcode.c (workflow step 2)

The `protocol_main_loop()` receives motion commands (if any) and handles them to `protocol_execute_motion_data()`; then refills the stepper buffers (segments and steps).
...
The code is mostly the same of the old interrupt handler (revisited to skip `sys` and `hal` calls, as well as probe and spindle code; they are in the new interrupt handler).
...
The reference board for testing this grbl core is [RPi Pico and the customized board driver](https://github.com/mfp20/grblHAL_RP2040_dualcore).
